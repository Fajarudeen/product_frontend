// import React from 'react'
import React, { useEffect, useState } from 'react'
import axios from 'axios';
import Row from 'react-bootstrap/Row';
import Col from 'react-bootstrap/Col';
import { Image } from 'react-bootstrap';
import Button from 'react-bootstrap/Button';
import { useParams, useNavigate } from "react-router-dom";
import { toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { ToastContainer } from 'react-toastify';



function ViewProduct() {
    const defaultImageUrl = 'data:image/jpeg;base64,'

    const params = useParams()
    const navigate = useNavigate()

    const [ProductList, setProductList] = useState([]);

    const handleDelete = () => {
        // Send a DELETE request to backend endpoint with the item ID
        axios.delete(`http://localhost:5000/api/product/deleteProduct/${params.id}`)
            .then((response) => {
                toast.error(response.data.message);
                setTimeout(() => {
                    navigate('/');
                }, 5000);
            })
            .catch((error) => {
                toast.error(error);
            });
    };

    useEffect(() => {
        axios.get(`http://localhost:5000/api/product/getItems/${params.id}`)
            .then(response => {
                setProductList(response.data.productItmById);
                // console.log(ProductList);
            })
            .catch(error => {
                console.error('Error fetching data:', error);
            });
    }, [params.id, setProductList]);     // Add 'params.id' and 'setProductList' as dependencies

    //   console.log(ProductList); // Will show the previous value, not the updated one

    return (
        <>
            <ToastContainer />
            <Row className='p-3'>
                <Col lg={3}>
                    <Image src={ProductList?.files ? `http://localhost:5000/uploads/${ProductList?.files[0]?.filename}` : ""} fluid
                        onError={(e) => {
                            e.target.src = defaultImageUrl; // Display the default image on error
                        }} />
                </Col>
                <Col>
                    <h1><span className='text-info'>{ProductList.productname}</span></h1>
                    <p className='mt-3 text-secondary'>Price: {ProductList.price} only</p>
                    <h6 className='mt-3'>Specifications: {ProductList.productdesc}</h6>
                    <Button variant="outline-primary mt-5" type="submit" href={`/update/${ProductList._id}`}>Update</Button>&nbsp;&nbsp;
                    <Button variant="outline-danger mt-5" onClick={handleDelete}>Delete</Button>
                </Col>
            </Row>


        </>
    )
}

export default ViewProduct
